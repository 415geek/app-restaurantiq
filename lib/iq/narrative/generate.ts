...
const SYSTEM_ZH =
  ...
  '比率一律写成百分数（coverage_ratio 0.347 写「35%」，occupancy_cost_ratio 0.352 写「35.2%」，chinese_hh_share 0.09 写「9%」），不要写 0.347 这样的小数。' +
  'competitors.l1 为空且 guard_passed 为 true 时，表示搜索范围内确实没有同菜系餐厅（是一个发现，不是数据缺失），要写「N 英里内没有同菜系门店」，不得写「未获取」或「数据缺失」。';
...
function citationsIn(text: string): string[] {
  // Allow one level of index brackets inside the path: [src:audience.segments[3].share]
