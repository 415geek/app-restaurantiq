...
function validFor(m: ReportModel, pageId: PageId): { title: string; body: string; refs: string[] } {
  ...
  const conds = pageId === 'page_2' ? `签约前条件：${m.score.conditions.map((c) => c.text_zh).join('；') || '无'}。` : '';
  return { title: '需求不足，不建议签约', body: `按 [src:${key}] 判断需求不足。${conds}`, refs: [key] };
}

test('(a) llm: null → all 14 pages are templates and every template passes NumberGuard on its own fragment', async () => {
  const m = load();
  const { narrative, stats } = await base(m, null);
  assert.equal(Object.keys(narrative).length, 14);
  assert.deepEqual(stats, { llm_pages: 0, template_pages: 14, regenerated: 0, guard_failures: [] });
  ...
  assert.equal(r2.stats.template_pages, 14);
...
test('(b) fabricated number on the first call, valid on the second → regenerated once, guard ok_after_regen', async () => {
  ...
  assert.equal(stats.template_pages, 13);
...
test('(d) a throwing or hanging llm → template fallback, never throws', async () => {
  ...
  assert.equal(r1.stats.template_pages, 14);
...
test('(f) cost ledger increments per LLM call; page 2 runs last on the summary tier', async () => {
  const m = load();
  const order: PageId[] = [];
  ...
  assert.equal(stats.llm_pages, 14);
  ...
  assert.ok([...tiers.entries()].filter(([id]) => id !== 'page_2').every(([, t]) => t === 'page'));
  assert.equal(cost.entries().length, 14);
  assert.equal(cost.entries().filter((e) => e.source === 'llm').length, 14);
  assert.equal(cost.total(), Math.round((13 * NARRATIVE_COST_PAGE_USD + NARRATIVE_COST_SUMMARY_USD) * 10_000) / 10_000);
