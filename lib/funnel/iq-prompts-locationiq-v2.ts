...
export function locationIqV3PremiumExtensionsBlock(lang: Locale): string {
  if (lang === 'zh') {
    return [
      ...
      '- site_history：必须依据上方「该地址过往/现有商家」实测块（若提供）：填 prior_business_name / prior_business_status / review_themes_positive / review_themes_negative / lessons_for_new_operator / note；餐饮商家已永久关闭 → prior_failures_detected=true 且 decision_tier 不高于 go_with_conditions；无实测块时才可用 [估算] 并降 confidence。',
  ...
  return [
    ...
    '- site_history: MUST be grounded in the [BUSINESSES AT THIS EXACT ADDRESS] block when present: fill prior_business_name / prior_business_status / review_themes_positive / review_themes_negative / lessons_for_new_operator / note; a permanently closed food business here → prior_failures_detected=true and decision_tier no higher than go_with_conditions; only without that block may you [estimate], and lower confidence.',
