...
- Paid full report generation entry point: **`generateIqFullReportWithN8nFallback`** in `lib/funnel/iq-generate-full-report.ts` (n8n when `getFullReportWebhookUrl()` is set, else OpenAI); **`fulfillIqPaidPurchase`** in `lib/funnel/iq-complete-purchase.ts` centralizes marking paid + generating `full_report_json` for both the webhook and the success URL.
