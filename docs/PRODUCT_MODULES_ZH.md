...
## 底层重构 · 截断细分在线上从未生效（2026-09-18）
- **缺陷**：§3.1 的细分预算写成「总上限 − 计划步数」。线上 `google_places_max_calls: 14`，而 egg_tart 计划 12 步、hunan 13 步、hot_pot 14 步，剩余 0–2 次；一次四象限细分需要 4 次，**于是线上从未细分过任何一个触顶的检索**。此前的实测都手动传了 `maxCalls: 24`，恰好把这个问题遮住了。
- **修复**：细分改用独立预算 `google_places_max_refine_calls: 32`，并按层排优先级——`direct` → `substitute` → `brand_anchor` → `l3` → `l4`。竞品层没穷尽会漏掉真正的对手，锚点层没穷尽只是背景计数糊一点。细分改为「整个计划跑完后再统一分配预算」，而不是谁先触顶谁先花。
- **实测效果（唐人街 900 Grant Ave，线上参数）**：竞品池 137 → **190**，同品类层 50 → **123**（2.5 倍），Golden Gate Bakery 仍在 Layer 1。Clement St：池 151 → 158，Breadbelly 仍在池内。
- **成本上限**：`data_cost_cap_usd: 0.10` 与 `report_cost_cap_usd: 0.50` 此前**只是声明，没有任何代码执行**。细分是整个计划里唯一不封顶的部分，41 次调用 × $0.032 = $1.31,是数据上限的 13 倍。现在细分会对照账本检查 `data_cost_cap_usd`：Places 还在 Google 免费额度内时 `perCallCost` 为 0，该限制不触发；一旦 `GOOGLE_PLACES_BILLED=1`，细分到上限即停，剩余单元如实标记为未穷尽，而不是悄悄超支。
- **仍未解决**：唐人街仍有 18 个单元触顶（多为第二层的叶子格），Clement 9 个。两处的 `pool_truncated` 都为 true，INV-1 因此不会出「品类空白」断言。其余成本上限（`report_cost_cap_usd`）仍然没有执行代码。

## D13 · Yelp 作为第二检索源 + 首次真实端到端报告（2026-09-18）
- **为什么需要第二个源**：Google Text Search 匹配的是「店叫什么」。在 900 Grant Ave 实测，`"egg tart"` 返回 **0** 条,因为「Golden Gate Bakery」的英文名里没有任何蛋挞字样;同一个查询在 Yelp 返回 **241** 条、该店排第一 —— Yelp 匹配的是「店卖什么」。两个引擎的失效方式不同,§3.2 因此要求否定性断言必须同时通过两者才允许发出。
- **`lib/iq/data/yelp.ts`(D13)**:按 §3.2 的别名集(中英双脚本,最多 3 条)各发一次 `/businesses/search`,半径 1600 m,结果并入同一个候选池、走同一套分类与去重。评分与评论数保留 Yelp 自己的口径 —— 同一家店 Yelp 3656 条、Google 1256 条,混算会得出两个平台都不认的数字。Yelp 单次上限 50 条,§3.1 的截断契约同样适用。无 key 时降级并写明「否定性结论仅有单一来源」,不静默消失。
- **接入时修掉两个自己引入的缺陷**:(1) 去重时 Yelp 的 id 被写进了 Overture 槽位,records 会谎称底图见过它,且 `by_source.yelp` 恒为 0;(2) Yelp 的类目别名(`bakeries` 复数)与 Google Table A(`bakery`)不一致,合并时会覆盖掉 Google 的类型,使该店掉出 Layer 1 —— **加了一个数据源反而让竞品数变少**(直接竞品 13 → 8,还混进一家意大利糕点店)。`YELP_ALIAS_TO_TABLE_A` 做了映射后恢复。
- **实测净效果(900 Grant Ave,线上参数)**:竞品总数 138 → **156**,同品类层 125 → **143**,**51 家**带 Yelp 出处,直接竞品维持 13 家。
- **首次跑通真实端到端报告**:D1 Census 地理编码命中 → block group 0607506110;D6 执行 41 次调用、189 个地点;D13 Yelp 68 家。在 D2/D5/D8/D11 均失败(无 Supabase、无搜索 key)的情况下,完整度 **15**,判定 **NO_GO**,系统没有输出任何自信的结论 —— §8 的最终验收标准成立。
- **由此暴露的真问题**:完整度 15 时,报告对客户说的是「不建议」,而诚实的答案应该是「数据不足,不予判定」。`verdictCap` 只下调不上调,而 51 分本身已落在 NO_GO 区间,所以第三档退化成了「不建议」。**把「我们无法判断」说成「别做」是 INV-5 的违反**,需要第四种判定徽章才能真正解决。

## 截断的代价改为按比例计算（2026-09-18）
- **问题**：把「竞品检索触及上限」一律判为竞品项零质量，并额外对判定硬封顶，是双重扣分且不成比例。唐人街、圣盖博谷、法拉盛的检索**必然**触顶——那是本产品的核心市场——于是「找到了 156 家竞品但无法证明找全」被当成零质量数据，**这些市场的任何地址都永远拿不到「可做」**。
- **改法**：截断的代价只通过完整度体现，不再额外封顶（`verdictCap` 去掉 `poolTruncated → null`）。而完整度里的代价按「已经找到多少家」分档：找到 ≥ 12 家 → 竞品项按 0.6 折算；不足 12 家且未穷尽 → 该项为 0，因为这时漏掉的很可能正是决定结论的那几家。
- **效果**：所有源齐全、用户三项齐全为完整度 100。同样条件下唐人街(截断、已找到 156 家) → **90**，评分够即可判「可做」；截断且只找到 3 家 → **75**，上限「有条件可做」。
- INV-1 不变：竞品池未穷尽时，「品类空白」这一具体断言仍然一律判 `unknown`。封顶的是「我们没找到所以不存在」这句话，不是整份报告的判定。

## D2 失败根因：Census API 已强制要求 key（2026-09-18）
...
## 落地页与免费结果页：数据出处收敛、选填入口加强、结果页语言切换（2026-09-20）
- **数据出处不再点名**：落地页首屏信任条、FAQ「数据从哪来」和页脚都去掉了具体数据供应商的名字，改为「官方公开统计数据 + 授权的商业地图与门店数据」的表述，并把重点放在「按你输入的具体地址拉取、每个数字都标注更新日期与许可、拿不到就写未获取」上。副标题同样改为「基于官方统计数据与授权地图数据」。报告内部的数据来源页不变——付费报告仍逐项列出机构、日期与许可，那是可溯源承诺的一部分；收敛的只是营销页面上的对外披露。
- **「补充租金和面积」改成真正的控件**：原先是 CTA 下面一行灰色小字链接，用户根本看不到。现在是一整行虚线绿边的按钮，带 + / − 圆标和展开箭头，展开后变为实线灰底的「收起租金和面积」。三种语言文案同步调整。
- **已生成分析数**：`/api/iq/stats` 返回 **6547 的基数 + 数据库实时行数**（当前 26，因此显示 6573），之后每生成一份分析自动 +1；数据库不可用时只返回基数，不再隐藏。基数是产品负责人给出的、表开始计数之前已经生成的分析数。
- **免费结果页新增语言切换（EN / 中文 / ES）**：放在结果页顶部、社会证明徽章同一行。切换不是换个标签——免费结果的标题、市场快照、关键风险都是按分析语言生成的，所以切换会**用新语言重新跑一次免费分析**（服务端按语言做 24 小时幂等，切回去不会重复生成），同时先把语言写入 cookie / localStorage，于是后续所有页面、结账时传的 `language`、以及解锁后的 360° 报告全部跟着切换。已经填的租金与面积随 URL 带过去；其余补充项需重填。

## 免费结论提速：并行取数、延后联网检索、承诺改为 1–2 分钟（2026-09-20）
- **现象**：落地页承诺「60 秒出结论」，实际一次免费分析走完要 100 秒以上。
- **根因**：免费分析的取数是一条串行链——地理编码 → 竞品检索（计划 12–14 次调用，加上 §3.1 细分最多 32 次，**全部一次接一次地发**）→ 步行距离 → Yelp/Foursquare → 人口普查（FCC 定位 + 两次 ACS）→ 联网检索（Tavily advanced，最长 45 秒）→ 财务模型 → 大模型撰写。而免费简报只读地理编码、竞品摘要与 ACS，**联网检索的结果免费结论根本不用**，却等在大模型之前。
- **改法**（内容与判定逻辑不变）：
  - `google-places.ts`：同一批可以一起发的调用并发发出（上限 6 路）。只有带语义的顺序保持串行——「先近后远」的加宽判断仍在此前所有调用合并完成后才做；细分按「层 × 深度」逐轮进行，一轮内的子格并发。首个调用单独探路：key 被禁用时不会一下子记账 6 次。已发未回的格子按已花费计入成本上限校验。
  - `iq-market-data-resolve.ts`：先做地理编码，随后**竞品检索、人口普查、联网检索三条慢腿并行**；新增 `onWebResearch` 回调，调用方可接管联网检索的 Promise。
  - `analyze/route.ts`：免费路由接管联网检索，让它与大模型撰写同时进行，写库前最多再等 5 秒合并进 `market_data_json`（付费报告照旧能读到；没等到就由付费阶段补取）。新增分阶段计时日志 `[funnel/analyze] timings {concept, cache, market, llm, web, insert, total}`，生产环境的真实耗时以此为准。函数上限显式设为 180 秒。
- **文案与进度条**：所有「60 秒」承诺改为「1–2 分钟」（落地页标题轮播、副标题、信任条、三步说明、FAQ、客服气泡）。免费进度曲线按真实节奏重排（40 秒到 60%，95 秒到 86%），去掉了 2.4 秒就把进度顶到 58% 然后停一分钟的阶梯。付费报告进度行的文案去掉了供应商名字（「检索周边竞品」「拉取周边人口与收入」）。
- **预期**：竞品检索从约 20–30 秒降到 8–12 秒，普查与联网检索不再串行排队，免费结论整体约 40–60 秒。是否稳定压进 60 秒，看部署后日志里的 `timings` 再决定要不要把承诺改回「约 1 分钟」。

## 视觉系统重做：暖白画布、藏青主按钮、去掉「AI 生成感」（2026-09-20）
- **目标**：整个 `/iq` 漏斗从深色仪表盘风格改为 Owner.com 一类的产品化浅色设计。判断标准是「像一家咨询公司交付的工具」，而不是「像 demo」。内容、判定逻辑、接口一律不变。
- **视觉系统**（`components/iq/ui.ts`，所有页面共用一套类名）：一个画布（暖白 `#F6F6F2`）、一个墨色（藏青）、一个信号色（绿，只用于状态与正向数字，不再做整张卡片的底色）。卡片一律白底细描边加极短的阴影；主按钮藏青白字，次按钮白底描边；分段控件（语言切换）藏青实心为选中态；判定徽章改为浅底深字（浅绿 / 浅蓝 / 浅黄 / 浅橙 / 浅红），在白底上可读。去掉的东西：渐变、发光光斑、玻璃态半透明卡片、emoji 图标（📊 ⚠️ 🔒 📤 🔑 ✅ 💾 均换为线性 SVG 或直接删除）、模糊遮挡的「████」付费预览行（改为带锁图标的普通列表）。
- **落地页**：导航与首屏改白底藏青字（原为深色发光首屏）；「三步」区改浅灰画布 + 白卡；深色只保留两处——专业版价格卡与页尾 CTA 带——作为节奏，不再是整页底色。深色面上的主按钮保持绿色，白色面上的主按钮一律藏青，这是唯一的一条颜色规则。
- **免费结果页**：从「居中大标题 + 渐变卡片」改为「备忘录式」左对齐布局：小标签 → 判定徽章 → 标题 → 副标题 → 地址与业态一行；市场快照用分隔线列表；关键风险用一条左侧红竖线而不是整块粉底；付费区把锁定项、三项核心输入、解锁按钮、access code 放在同一张白卡内。进度卡、业态确认卡、留邮箱底板、客服气泡全部同步改浅色。
- **付费报告壳**：报告纸页下方的画布由藏青改为暖白（仅 `@media screen` 下的 `.report-viewer` 规则，`/print` 与服务端 PDF 不受影响）；纸页阴影收敛；下载 / 分享 / 打印版 / 语言 / 补充信息页脚改为白卡；「保存报告」账户卡、生成等待页、成功 / 取消 / 登录 / 我的报告页同步改浅色；登录页的 Clerk 组件本地覆盖为浅色外观（根布局仍是深色 App 壳）。
- **未动**：旧版（无 360° 模型时）的 `ReportContent` 仍按深色样式渲染，因此在报告页上给它保留了一块藏青底，等它退役再处理。

## 配色微调：撤掉信号绿，改用同一族的五个状态色（2026-09-21）
- **问题**：暖白 + 灰的底子配上亮绿（`#22C55E` / emerald-500）不搭。那支绿是高饱和的冷绿，而画布 `#F6F6F2` 偏暖，两者不在一个色温上；加上它当时被当装饰用（小圆点、序号 01–06、眼眉标题、输入框聚焦环、「补充租金」的虚线框、价格卡的发光描边、两处主按钮），页面上到处是与含义无关的绿，正是「AI 生成感」的来源。
- **规则改为**：**绿色只在表示「这是好的」时出现，不做装饰**。结构性元素——按钮、圆点、序号、聚焦环、徽章、展开控件——一律藏青或中性灰。
- **状态色改为同一配方**（`app/globals.css`）：一支深而低饱和的墨色承载含义，底色和描边由同一支墨按 8% / 25% 透明度混出，因此五个状态天然同族，落在白卡或暖白底上都成立：松绿 `#1C5D45`（可做）、钢蓝 `#2E4C6D`（有条件）、赭黄 `#8A5A12`（谨慎 / 数据不足）、赤陶 `#9A4F1F`（高风险）、砖红 `#98342B`（不建议）。原先散落的 `rose-700` / `amber-800` / `emerald-50` 等 Tailwind 调色板全部并入这五支。
- **评分条**同步改为三支中间调（`#3E7F63` / `#C08A2E` / `#B4544A`）。该函数只在屏幕端使用，**PDF 与 `/print` 不受影响**。
- **两处深色卡**（专业版价格卡、页尾 CTA 带）的按钮与徽章从绿改为白底藏青字，去掉价格卡外围的绿色发光环——深浅对比本身已经够强，发光环是多余的。
- **令牌整理**：画布统一成一支 `--brand-canvas: #F6F6F2`（此前布局用 `#F6F6F2`、板块用 `#F7F8F4` 两支近似色）；新增 `--brand-paper`（卡内浅底）与 `--brand-navy-soft`（藏青按钮的 hover），替换掉散落在各文件里的 18 处字面色值。删除无人引用的 `--brand-emerald` / `--brand-lime`。
- **分享弹窗**（`ShareModal`）此前仍是旧的深色版，从浅色页面弹出很突兀，一并改为白卡。
- **Logo 的橙 `#FF6D36` 保留**：它只出现在标志本身，而暖橙与暖白画布同色温，比原来的冷绿更协调，现在读起来像是有意为之的品牌色而不是冲突。
- **顺手修掉**：`IqLanding.tsx` 里 `phrases.join()` 用的是一个真实的空字节（U+0000），导致 grep / diff 把整个文件当二进制；改写成 `'\u0000'` 转义，行为不变。
