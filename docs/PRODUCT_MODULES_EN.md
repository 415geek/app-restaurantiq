...
## 360° fixes from the first production run (2026-09-12)
- Google Places (New) rejects `bubble_tea_shop` / `hunan_restaurant` (not in Table A, INVALID_ARGUMENT): the plan now uses `tea_house` + `dessert_shop`, and direct competitors (L1) come from **Text Search (New)** for "<cuisine> restaurant" biased to 5 mi (Nearby's 20-per-call cap was hiding L1); bootstrap mode accepts a `partial` Google pool.
- When block-group C16001 Chinese-speaker counts are missing, the Chinese share and p_cn fall back to B02018 Chinese ancestry ÷ population (all four rings previously showed 「未获取」).
- Narrative guard accepts `[src:a.b[3].c]` citations; body caps widened to 200 / 560 chars with dot paths requested; `precheck_reasons` deduplicated. New `.github/workflows/report360-trigger.yml` (manual / push `.github/report360-queue.txt` to trigger generation).
- Second production run fixes: the 2023 ACS B02018 / B02015 layout changed (`_002E` = Chinese except Taiwanese, `_008E` = Taiwanese; the previous `_007E` was Okinawan) — corrected; narrative output caps lowered to 420 / 950 tokens with the 120-character body limit made hard in the prompt; page JSON fragments gain `_derived` counts (L1, L1+L2, alternatives, …) so NumberGuard accepts legitimate totals. New `.github/workflows/census-probe.yml` verifies Census variables from a networked runner.

## 360° report · seven customer-feedback items (2026-09-12)
- **Cost column removed**: the page-14 sources table no longer has 状态 / 获取 / 成本 columns and the footer no longer prints "report cost $x · elapsed"; no per-report cost appears anywhere in the panel or the report.
- **Competitor dedupe and accuracy**: `engines/competitor.ts` `dedupeCandidates` merges on three keys — shared `google_place_id` (any distance), same name ≤ 100 m, Chinese⇄Latin name at the same spot ≤ 60 m (Latin kept as `name`, Chinese as `name_zh`); page 7 `uniqueCompetitors` adds a presentation-level pass keyed on the normalised name in either language, so each business gets exactly one card (Chinese name first, English inline in smaller type).
- **Plain but professional Chinese**: every template in `narrative/templates.ts` and every page in `render/pages.tsx` rewritten; `plainZh()` sanitises legacy narratives at render time (walk10 / drive10 / L1 / L2 / Huff / P25 / HHI / β / coverage_ratio → 步行 10 分钟范围 / 开车 10 分钟范围 / 同菜系竞品 / 其他中餐 / 需求分流模型 / 低位·中位·高位 / 集中度 / 距离衰减参数 / 需求覆盖率); each necessary term is explained in parentheses once; `SYSTEM_ZH` gains the "written for a restaurant owner" audience rule and `paramNotes('zh')` tells the model the plain wording for every id.
- **Real map**: new `render/static-map.ts` (Google Maps Static API roadmap, muted style, four reachability polylines, numbered competitor markers in page-7 order, rail stations; `resolveStaticMaps` fetches server-side into data URLs and falls back to the SVG when the key is missing or the request fails); `render/map.tsx` adds `MapFigure` (`<img>` basemap + plain-language legend) and `app/print/[reportId]/page.tsx` passes `staticMaps` to `ReportDocument` (thumbnail on page 1, hero on page 3).
- **Paid intake form**: new `components/iq/PaidIntakeForm.tsx` (all optional, one minute; `embedded` mode above the unlock CTA on the free result page, `standalone` mode behind "补充信息并重新生成" in the 360° panel) and `POST /api/funnel/report-inputs` (`lib/funnel/iq-report-inputs.ts` zod validation, merged into `market_data_json.user_inputs`: rent, sqft, seats, dine-in / delivery ticket, delivery share, capex, parking, existing stores, known competitors ≤ 10, listing URLs, dayparts, notes). `SiteInput.known_competitors` drives ≤ 3 extra Google Text Searches (`buildGooglePlacesRequest`) and a post-dedupe L1 pre-classification; submitting forces a regeneration and polls until the new model lands.
- **Source-gap agent**: new `lib/iq/ops/source-gap-agent.ts` and ops task `source_gaps` (`vercel.json` cron Mondays 12:00 UTC): scan paid reports from the last 60 days for partial / failed sources → when a gap was caused by an unloaded table (LODES / iq_poi / snapshots) that is now loaded, regenerate and commit only if the `ok` source count grows and the tier does not drop to precheck (30-day cooldown) → for gaps with no in-house fix (reachability, foot traffic, rent, transit ridership, pipeline, Chinese-community signals) run ≤ 3 web searches per run and store government / open-data / clearly licensed candidates in the new `iq_source_candidates` table as `new` (never auto-adopted). Migration `0011_iq_source_candidates.sql` (also `iq_source_gap_runs`, RLS deny-all).
- **Page 14 becomes data lineage**: title "每个数字都可追溯到公开数据来源"; columns 数据源 / 内容 / 更新日期 / 来源机构 / 许可; degradations and gaps go into a plain-language 「数据说明」 footnote (no more "12 个数据源中 9 个完整" or red 未获取 badges); data completeness stays as a small "85/100" in the parameters box.
- **Page 15 summary**: new `page_15` (verdict badge + one-paragraph conclusion, the three deciding numbers: coverage ratio / occupancy cost / total score, must-do conditions verbatim from `score.conditions`, top-3 alternative cuisines, three next steps); the narrative layer treats `page_2` and `page_15` as summary tier and generates them last; QA gates, `smoke-print.ts` (new no_jargon / no_cost_line / no_source_count_title / page_15_is_summary assertions) and footers all use 15 pages.
- Misc: `charts.tsx` draws a zero-share label outside the bar (contrast); the ops route comment lists `source_gaps`; 360° panel copy now says 15 pages.
- **Cover page**: the paid report gains a dedicated unnumbered cover (brand bar, "商圈选址分析报告 · 360° Site Selection Report", edition tag paid / precheck, address and concept, real map, report id / generated / data-as-of / prepared-by and disclaimer); the former page 1 is renamed "报告概览 · At a Glance"; the PDF is 16 pages (cover + 15 analysis pages) and `smoke-print.ts` asserts 16 PDF pages with 15 body titles.
- **Third production-run fixes**: the candidate pool is capped at max(3 mi, drive 15) = 5 mi per §3.1 (Text Search is only *biased* to 5 mi and had pulled every Hunan restaurant in San Francisco into L1); NumberGuard accepts ratios above 1 written as percentages (coverage 1.72 → "172%"), which previously tripped "number not in JSON" and demoted the report to precheck.
- **"No same-cuisine competitor" is a finding, not a gap**: `competitors.pool_radius_mi` and `competitors.l1_nearest_outside_pool` (nearest same-cuisine restaurant beyond the candidate pool) are written to the model; page 7 shows "none within N mi, nearest is X at Y mi" with a void explanation when L1 is empty and the competitor guard passed, and only says 未获取 when the competitor sources failed; template and narrative prompt updated (ratios always as percentages; never 未获取 for an empty L1 with a passing guard). The cluster-curve label now reads 本址步行 10 分钟内 N 家; page 6 table padding tightened to avoid overflow.
- **Production PDF fix**: downloading the PDF returned "Browser was not found at /usr/bin/google-chrome-stable" — Vercel did not expose `VERCEL=1` and Fluid Compute sets no `AWS_LAMBDA_FUNCTION_NAME`, so the launcher took the local branch and fell back to a system Chrome. `lib/iq/render/chromium.ts` now detects serverless via `VERCEL_ENV / VERCEL_REGION / LAMBDA_TASK_ROOT / AWS_EXECUTION_ENV / /var/task`; when the bundled `bin/` is missing it downloads the matching release pack into `/tmp` once per instance (3 s locally); serverless never falls back to a system Chrome and the error names the real cause. `next.config.ts` tracing keys gain the escaped `[id]` form. The report page's "Print / preview" opens the light `/print/<id>` edition when a 360° model exists instead of printing the dark web page; the trigger workflow probes the server PDF once a report is ready.
- Follow-up: the production probe then failed with a missing `libnspr4.so` — `@sparticuz/chromium` extracts its Amazon Linux 2023 shared libraries and sets `LD_LIBRARY_PATH` at module load only when `AWS_EXECUTION_ENV` names a Node 20+ runtime or `VERCEL` is set. The launcher now imports the package lazily and sets `VERCEL=1` first when serverless is detected; verified locally that `/tmp/al2023/lib` is extracted and the launch succeeds.

## Landing page redesign (2026-09-14)
- `app/iq/page.tsx` rebuilt in the Owner.com / DoorDash product-led idiom: white canvas, one bold headline, an address "search card" as the single hero action (address + cuisine side by side, orange primary button, rent / size folded away as optional), a trust strip (60-second verdict · data sources · free without sign-up); then four real rendered report pages (cover / trade-area map / direct competitors / summary from the golden Millbrae fixture, stored in `public/marketing/iq/`), a three-step flow, a six-card "what's inside", sample numbers (fixture data, assumptions stated), free vs professional pricing (reads `NEXT_PUBLIC_STRIPE_PRICE_USD`, default $19), audiences, an FAQ accordion, the account entry and a disclaimer footer. One-tap zh/en toggle (Chinese default), smooth-scroll nav anchors, single column on phones with no horizontal scroll. Brand colour stays `--brand-orange`; the logo uses the transparent SVG mark.
- Hero headline becomes "fixed lead + typewriter tail": `Before you sign the lease, know` + an orange, character-by-character rotating phrase (money / Chinese households nearby / same-cuisine rivals / break-even / rent share). Single line on desktop (`clamp` size + `md:whitespace-nowrap`), lead and typed phrase on two lines on phones; the longest phrase's width is reserved so the line never jumps; SSR shows the first phrase in full and `prefers-reduced-motion` disables the animation.

## Brand colour system · Midnight Navy + Signal Green + Warm White (2026-09-14)
- Tokens: Midnight Navy `#0B1220` (brand / dark surfaces), Signal Green `#22C55E` (CTA / opportunity / good location), Emerald `#16C784`, Warm White `#F7F8F4` (light canvas), Slate `#334155` (body), Amber `#F59E0B` (caution), Red `#EF4444` (risk), IQ Lime `#A3FF4F` (advertising only). Exposed in `app/globals.css` as `--brand-navy / --brand-green / --brand-emerald / --brand-lime / --brand-canvas / --brand-slate / --brand-amber / --brand-red` (Tailwind `bg-brand-*` / `text-brand-*`); the old `--brand-orange` remains for legacy surfaces only.
- Landing: nav and hero on Midnight with white headline, green caret and green CTAs, white search card, warm-white sections; the headline is now a full-sentence typewriter — `签 lease 前，先通过数据了解该商圈是否适合` always first, the other five in a random order per visit (Fisher–Yates), one line on desktop, two on phones. The `/iq` shell background is Midnight; the 360° panel uses green CTAs.
- Report (`/print` + PDF): `print.css` and `render/format.ts` palettes moved to Midnight ink, Signal Green accent and warm-white panels; the verdict badge is coloured by outcome (GO green / conditional amber / NO GO red, dark ink for contrast); the map's site marker is green, reach rings Midnight, Chinese anchors blue so they never read as the site; 4.5:1 contrast and 16-page overflow checks pass on the golden fixture. The four homepage report screenshots were refreshed.
- **Professional-report Chinese typography**: titles in Noto Serif SC (思源宋体, 700 / 900: cover title, per-page action title, section h2), body and tables in Noto Sans SC (思源黑体), numerals in Inter — the pairing used by Chinese consulting and brokerage research reports; loaded via Google Fonts on `/print` and `/iq` with system CJK fonts (PingFang / Microsoft YaHei / SimSun) as fallback. The landing headline and section titles use the serif as well. The trigger workflow now uploads the probed production PDFs as a 7-day artifact so real font rendering can be checked.
- Landing: the report-screenshot section and its nav item are removed (`public/marketing/iq` deleted); the headline now types its first sentence from empty on load (blinking caret), holds, erases and types the next in random order, and is no longer disabled by the OS reduce-motion setting; SEO / no-JS readers get the full title via sr-only text.
- Landing address placeholder now uses the generic example "123 Main St, San Francisco, CA 94105" (with ZIP) instead of a real customer address.

## Support bubble (2026-09-14)
- `components/iq/SupportBubble.tsx` sits bottom-right in the `/iq` layout, so it is on every funnel page (landing, free result, paid generating, report). Scripted, no LLM. Core scenario: a customer refreshes / goes back after paying — the generating and report pages write the report id to `localStorage` (`iq:last_paid_report`, 30 days); the bubble verifies it is paid via `POST /api/iq/support/recover` and hands out a "back to the report" link (generating / ready); without a remembered id it asks for the checkout email and looks up paid reports by `customer_email / notify_email` (≤ 5, paid only). Three canned answers: generation time, PDF download, payment problems.
- "Talk to a human" button opens `https://wa.me/<number>` (WhatsApp) with the report id and address pre-filled; the number comes from runtime config `SUPPORT_WHATSAPP` (`iq_settings`, international digits), falling back to a `SUPPORT_EMAIL` mailto, or a "not connected yet" note when neither is set. `GET /api/iq/support/config` serves both. Chinese by default, `?lang=en` or the in-panel toggle for English.
- Paid-report generating page: the "N s elapsed" line under the progress bar is replaced by `components/iq/GenerationTicker.tsx` — address-aware status messages in 8 phases that follow real progress, five wordings per phase rotating every 5 s (after its own lines a phase borrows from the next two, so a minute shows at least 10 different sentences and never the same line twice in a row) ("Pulling the history of businesses at this address…", "Analysing population and Chinese households around San Francisco…", city parsed from the address); after 15 s a countdown appears on the right ("About 2:30 to go") anchored on the typical 3-minute run and only ever decreasing: under a minute it becomes "Under a minute to go", past 3 minutes "taking a little longer — finishing…" (no more 5-minute stretch that jumped back up); the start time is remembered per report id in localStorage for 30 min, so a refresh or language switch continues the same countdown and only an explicit retry restarts it, and "Finishing up" at ≥ 90 %, never negative. The progress bar gains a `statusLine` slot; the free result page is unchanged.

## Trilingual site · English default / 中文 / Español (2026-09-14)
- Locale contract `lib/i18n/locale.ts`: `Locale = 'en' | 'zh' | 'es'`, default **standard U.S. English**; `toLocale()` normalises any input, `pick(lang, {en, zh, es?})` picks copy (missing Spanish falls back to English, never to Chinese), `LOCALE_TAG` (en-US / zh-CN / es-US) feeds `<html lang>` / `<main lang>`.
- Resolution (`lib/i18n/resolve.ts`, `server-locale.ts`, `use-locale.ts`): server pages use `?lang=` → the report row's `language` column (report / success pages) → `iq_lang` cookie (1 year) → `Accept-Language` → `en`; the client uses `?lang=` → localStorage / cookie → `navigator.language` → `en`. A chosen locale is persisted to both cookie and localStorage, and every link, form, e-mail link, Stripe `success_url / cancel_url`, PDF and print link carries `?lang=`; the hosted Stripe Checkout page and product name follow the language too.
- Funnel UI fully trilingual: landing (`components/iq/IqLanding.tsx`, EN / 中文 / ES switcher, six Spanish typewriter headlines), free result, paid generating page with ticker / countdown, report page, success / cancel / login / dashboard, support bubble, share components, social-proof stats, report-ready e-mail. Every dictionary is `Record<Locale, …>` so a missing language is a compile error; English copy polished to standard U.S. English.
- Legacy LLM report (`lib/funnel/**`): every prompt writes in the visitor's language via `LANGUAGE_INSTRUCTION[lang]` (en: standard U.S. English; es: neutral Spanish); enrichers emit `_es` siblings and components / PDF read them through helpers (`competitorTakeaway`, `demographicNarrativeParagraph`, `siteHistorySummary`, `financeArchetypeLabel`…) with English as the Spanish fallback. The legacy HTML→PDF download is trilingual as well.
- 360° report (`lib/iq/**`): `meta.language` accepts `es`, `narrative_language` records the language actually narrated; `lib/iq/render/i18n.ts` centralises all 15 pages' labels, legends, charts, cover, source table, disclaimer and verdict words (GO / CONDITIONAL GO / NO GO ↔ 可做 / 有条件可做 / 不建议 ↔ VIABLE / VIABLE CON CONDICIONES / NO VIABLE); template narratives `templateZh/En/Es` share identical numbers and refs, LLM narratives get a `SYSTEM_ES` prompt, Spanish number guard and banned-word list; `lib/iq/narrative/plain.ts` translates the engine's driver / risk / condition phrases into all three languages. `/print/<id>?lang=` and `/api/iq/report/<id>/pdf?lang=` override the stored language; when the stored narrative language differs from the request the locale template is used. en / es drop the English subtitle of the zh layout and compact via `[data-lang]` CSS; all 15 pages fit in all three languages (smoke `--lang en|zh|es` passes).
- Not covered: prompts inside the n8n workflows still have only zh / en branches; the other product modules (delivery, ops…) remain bilingual zh / en.

## 360° report · no assumed rent / pre-payment intake / report page = PDF (2026-09-14)
- **No invented rent**: `lib/iq/engines/finance.ts` drops both estimation branches (sqft × comp $/sf, sqft × tier $/sf). With no monthly rent, `fixed_cost.rent = null`, `rent_source = 'not_provided'`, `finance.rent_excluded = true`; fixed-cost total, break-even, safety line, scenario vs break-even and payback are computed **excluding rent** and labelled "(excluding rent)" everywhere (pages 2 / 7 / 9 / 10 / 15, twin bars, coverage gauge, sensitivity waterfall, template narratives and LLM prompts in all three languages); occupancy-cost ratio is null, the rent risk is gone and a high-probability "Monthly rent not provided: occupancy cost cannot be assessed" risk takes its place; the "Rent +10%" sensitivity becomes "Each +$1,000 rent → break-even +$X". New `finance.max_rent_for_10pct_usd` (captured demand × 10%) is the **rent ceiling** key number on pages 10 and 15, the pre-signing conditions always include "Add the actual monthly rent and regenerate … rent ceiling about $X", the financial dimension scores a neutral 50 and the verdict is capped at CONDITIONAL GO (alternatives too). The page-10 rent row reads "Not provided · add the monthly rent under “Add details” and regenerate". `scripts/smoke-print.ts --no-rent` reproduces this edition and asserts: the fixture's $17,000 appears nowhere, the ex-rent and rent-ceiling labels are on pages 10 / 15, no GO badge, no overflow. Already-stored reports keep their stored numbers until regenerated.
- **Pre-payment intake**: `components/iq/PaidIntakeForm.tsx` gains monthly rent (`monthly_rent_usd`) and size (`sqft`), which with seats form a pinned "three numbers that matter" group, each with a one-line consequence ("Leave blank and the report assumes no rent: it shows a rent-excluded break-even and a rent ceiling instead"). The paywall card on the free result page is now two steps: **Step 1 · Three numbers make the report accurate (30 s, optional)** always expanded (the other fields fold under "More optional details"), **Step 2 · Unlock the full risk audit — $19**. Rent / size entered on the landing page are prefilled and marked "already provided ✓". With rent empty, the first unlock click only shows a notice ("No monthly rent entered: the report will not assume one…", with "Add rent / Pay anyway"); the second click pays. Payment is never blocked.
- **Report page = PDF layout**: `/iq/report/[id]` renders the very same `ReportDocument` as `/print` when a 360° model exists (cover + 15 white pages on the Midnight Navy background); `components/iq/Report360Document.tsx` scales it to the container width (≈0.46 on a 390 px phone, no horizontal scroll, 1:1 on desktop). Under the last page `Report360Footer` offers: Download the 360° PDF, Share, Print edition, EN / 中文 / ES pills, Add details & regenerate (saves, forces a rerun, reloads automatically), Analyze another address. The legacy "Download the formal report / Print–Save as PDF" and "Regenerate professional report" blocks are removed (`ReportActions.tsx` deleted); without a model the legacy content + `Report360Panel` still show and the page reloads into the document once generation finishes. `?fixture=millbrae` previews it outside production.

## Review Spec v2 · single conclusion source / competitor retrieval / text quality (2026-09-17)
- **Single conclusion source (P0-A)**: the web report and the PDF each computed their own numbers, so one report disagreed with itself on the overall score, verdict, break-even, safety line, fixed cost, utilities / insurance / POS, occupancy-cost ratio and data confidence. New `lib/iq/conclusion/**`: `conclusionFromModel(model)` in `conclusion.ts` is the **only** place a verdict or break-even is derived (`verdictFromScore`, thresholds `go: 70 / conditional: 55` read from `defaults.yaml`, and the page-11 rule text is generated from the same constants); `display.ts` holds the client-safe half so no `'use client'` file reaches `node:fs`; `reconcile.ts` runs inside `reportModelSchema.transform`, so **every parse** aligns the model to the stored conclusion and the print / PDF path physically cannot render a number that disagrees with it; `schema.ts` carries a zod schema compile-time checked against the interface.
- **One tier cost table**: `defaults.yaml` gains `cost_scale` (hcol / mcol / lcol tiers with wage, multiplier and income threshold, plus `hcol_states`, `labor`, `split` and per-archetype `concepts`). Every hard-coded wage / utilities / insurance / POS / marketing / misc literal is gone from both finance engines (`lib/funnel/iq-finance-model.ts` and `lib/iq/engines/finance.ts`), which now read that one table; concept→archetype resolves once through `archetypeIdFor` (previously `skewers` and `middle_eastern` landed in different archetypes per engine). The web engine also loses its fourth `hcol_metro` tier, its per-archetype safety multiplier and its **rent estimator**, matching the shipped "never invent rent" rule.
- **Ordering**: `stageDraft` now calls `ensureConclusion()` first — reusing a stored conclusion, otherwise running `generateReport360ForRow(id, { narrative: false })` against a 150 s budget — and feeds the conclusion into both the drafting prompt and finalize; on timeout or failure it sets `conclusion_pending` and falls back to the old `applyFinanceModelOverride`. The 360° pass that follows is narrative-only (`generateReport360Narratives()`, with `meta.narrative_pass` making it run exactly once and never recompute). On the web side `applyConclusionOverride` overwrites the overall score, decision tier, layer scores, break-even, safe revenue, cost breakdown, confidence, occupancy ratio, decision matrix and the three revenue scenarios; the LLM keeps prose only.
- **Competitive intensity and counts**: `dashboard.competition_intensity`, `risk_audit.competition_pressure_score` and the whole decision matrix now come from the conclusion's `competitive_position` dimension, and the web competitor map prints `competitors.counts` / `market_data.summary.counts` instead of the length of its own whitelist — the same numbers the PDF shows.
- **Cuisine taxonomy fix**: the Cantonese roast-meat entry in `cuisine_taxonomy.yaml` drops the bare `BBQ` and `Roast` keywords, which matched "Seoul Garden Korean BBQ", American BBQ and coffee "Roastery"; only qualified forms remain (Siu Mei / Char Siu / Roast Duck / Cantonese BBQ …). `lib/iq/engines/competitor.ts` adds a rule to Layer-1 detection, ahead of the query-provenance gate: a name carrying the concept's own keywords is self-sufficient evidence. A place called "Golden Gate Egg Tart" should not be dropped just because the Layer-1 query did not return it and a category-mapping rule only saw a generic `bakery` type.
- **Report text-quality guard (P1-g)**: new `lib/funnel/iq-text-quality.ts` with two severities — `corrupt` (U+FFFD, private-use area, compatibility ideographs, CJK Ext A/B, control characters, and Latin fragments wedged between Han characters) and `suspect` (Han characters outside the project vocabulary: reported, never rewritten, so real restaurant names survive). The vocabulary is generated by `scripts/build-cjk-vocabulary.mjs` scanning the repo (`lib/funnel/iq-cjk-vocabulary.json`, 1,475 characters from 511 files). `scrubCorruptedReportText()` is folded into `stripInternalIqReportFields`, so every exit point gets sentence-level scrubbing, and a new `text_quality` QA gate (`lib/iq/qa/gates.ts`) passes the address, cuisine labels and L1/L2/L4 competitor names as legitimate context and fails only on `corrupt` findings.

## Review Spec v2 · wait experience (P1-f) (2026-09-17)
- **Tiered ETA (§4.7)**: the generating screen no longer shows one generic "3–5 minutes" for two tiers that are minutes apart. New `lib/funnel/iq-eta.ts` holds the pure ETA math; `iqRecentGenerationDurationsMs()` reads the wall-clock span (`generation_started_at` → `generation_updated_at`) of the newest completed runs of the **same tier** (filtered on `generation_state_json->>mode`), and `tierEtaFromHistory()` shows the **P50 of the last 30** of them, falling back to the static 4 min (standard) / 12 min (professional) below that sample count and rounding to whole minutes with a floor of 1 — never "0 minutes". The status endpoint returns it as `etaSeconds` / `etaSource`, cached ten minutes per tier so a 3 s poll stays one row read. **Monotonic countdown**: the wait screen owns the clock and clamps the remaining seconds against the last value shown (`monotonicRemainingSec`), so a longer tier ETA arriving mid-run can only pull the number down — the 1 min → 2 min jump is gone. The rotating status copy borrows from the neighbouring phases (forwards, and backwards for the late phases) until its pool holds at least twelve wordings, so a long stage never cycles between two sentences.
- **Email capture is always on**: `emailEnabled` is now unconditional (the address is stored on the row either way) with a separate `emailWillSend` saying whether a mail can actually go out, so the UI never promises one it cannot send. Past **five minutes** the card switches from an aside to an active prompt — "Leave your email and we'll send the PDF when it's ready" / 「留个邮箱，完成后把 PDF 发给你」 / "Déjanos tu correo y te enviamos el PDF cuando esté listo" — wired to the existing `/api/funnel/full-report/notify` → `notify_email` → `sendReportReadyEmail` path (Resend); no new provider.
- **Stage-stall detection and degradation**: `STAGE_STALL_MS` (5 min) sits above a stage's own ~250 s budget, and `runReportGenerationStage` now wraps each stage in a watchdog, so a job silent for the stall window is a dead invocation rather than a slow one. `deriveUiStages` flags the current row as `stalled` off the job heartbeat (not the row's start, which legitimately spans draft + verify), the checklist says the step is being retried instead of spinning, and the status endpoint re-kicks the worker from the last checkpoint. When a stage's attempts are spent, `canDegradeStage()` decides whether the run walks past it: `enrich` degrades to the stored market pack, `verify` to an unverified draft, `draft` only when an earlier attempt checkpointed one; `finalize` cannot degrade, so that one still fails the run. Degraded stages are recorded in `generation_state_json.degraded`.

## Jurisdiction data agent · evidence ladder and the hood conclusion (2026-09-17)
- **The problem**: the report described a commercial space with residential listing data — "built 1912, 8 bed / 6 bath Multi-Family, 4,685 sqft" was in fact the flats upstairs. The entire BrightData Zillow path is removed (`web_data_zillow_properties_listing`, `fetchZillowListing`, `real_estate_data` and its prompt anchor block), and a regression test scans every non-test file under `lib/iq/jurisdiction/**` and `lib/funnel/external-data/**`, failing on `zillow / zestimate / bedrooms / real_estate_data` and friends. LoopNet commercial listings (E5, rent comps only) stay, and the same test proves they cannot reach `property_facts`.
- **Evidence ladder E1–E5**: E1 food-facility permits / health inspections, E2 building and mechanical hood permits, E3 prior tenant (classified by `priorTenantSignal` in `site-history.ts` from the business name plus categories into `hot_kitchen / limited_food / non_food`), E4 assessor roll (year built, use code), E5 commercial listings. `hood_permit_found` can be set **only** by E2, and there is structurally no code path from E4's year built to any hood claim. `HOOD_INFERENCE_GUARD` states the rule in all three languages: never infer ventilation from building age, bedroom count or listing prose.
- **Jurisdiction registry**: `lib/iq/jurisdiction/**` (types, address parsing with injection-safe SoQL/ArcGIS escaping, registry, socrata + arcgis_rest adapters, normalizer, conclusion, discovery, entry point) plus migration `supabase/migrations/0012_jurisdiction_registry.sql` (deny-all RLS, seed rows via `ON CONFLICT DO NOTHING`). Resolution runs address → Census geocode → county FIPS → registry row → adapters in E1 → E2 → E4 order → normalize; responses cache through the existing market cache under a new `iq360_jurisdiction` source (7-day TTL) and cost is booked on the same ledger ($0 for open data).
- **An unregistered jurisdiction issues no requests**: `resolveJurisdictionFacts` makes **zero fetches** for one (asserted in a test) and returns `jurisdiction_coverage: 0` with every fact `null` plus a `fallback_reason`. `discoverJurisdiction` is a separate async entry point and is never called from the report path.
- **Connected today**: 06075 San Francisco, with three real Socrata datasets (food-facility inspections `pyih-qa8i`, DBI building permits `i98e-djp9` filtered on type i hood / grease interceptor / kitchen exhaust / ansul keywords, assessor secured roll `wv5m-vpq2`). 06081 San Mateo / 06085 Santa Clara / 06001 Alameda are **registered but deliberately unconnected** — their dataset ids were not verified, and an invented id is worse than a declared gap, so they carry `coverage_score` 0.15 and render as "permit data for this jurisdiction is not connected". Later batches are data rows, not code.
- **Three conclusion variants** (all trilingual): with a permit record → "this address holds or has held a food-service permit including a mechanical exhaust permit; conversion risk is low", and the pre-signing checklist becomes "verify the hood model on that permit is still in place"; prior-tenant evidence only → "the previous tenant ran a hot kitchen, so the space most likely already has commercial kitchen conditions, but no permit record confirms it — inspect on site"; no data → "permit data for this jurisdiction is not connected, so ventilation and power cannot be judged remotely", with "request the DBI permit record" added as a must-check item. `data_confidence_input` carries the confidence delta (permit record +4…+10, prior tenant +2, not connected −8) and trilingual acquired / missing data lists, each missing entry quoting its own gap reason.
