...
- Paid-report generating page: the "N s elapsed" line under the progress bar is replaced by `components/iq/GenerationTicker.tsx` — address-aware status messages in 8 phases that follow real progress, two wordings per phase alternating every 5 s ("Pulling the history of businesses at this address…", "Analysing population and Chinese households around San Francisco…", city parsed from the address); after 15 s a countdown appears on the right ("About 2:30 to go") anchored on the typical 3-minute run, stretching to 5 minutes near the mark, then "taking a little longer — finishing…", and "Finishing up" at ≥ 90 %, never negative. The progress bar gains a `statusLine` slot; the free result page is unchanged.
